...
document.addEventListener('DOMContentLoaded', function() {
    ...
    startPaymentButton.addEventListener('click', function() {
        ...
        if (selectedProduct && customerName && customerEmail) {
            ...
            alert('Please select a product and enter your name and email.');
        }
    });
    ...
    form.addEventListener('submit', function(event) {
        ...
        var amount = selectedProduct * quantity;

        fetch('/create-payment-intent', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ amount: amount, customer_name: customerName, customer_email: customerEmail })
        })
        .then(function(response) {
            if (!response.ok) {
                return response.text().then(text => { throw new Error(text); });
